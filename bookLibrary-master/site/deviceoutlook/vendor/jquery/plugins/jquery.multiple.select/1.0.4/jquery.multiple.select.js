 ...
 *
 * 30/08/2013
 *    add support for not showing the items selected, and show just an icon.
 *    example:
 *          $(".selectOptions").multipleSelect({iconized: true});
 *    Carlos Gomez
	...
	function MultipleSelect($el, options) {
		...
		$('body').click(function(e) {
			...
			if (($(e.target)[0] === that.$drop[0] ||
					$(e.target).parents('.ms-drop')[0] !== that.$drop[0]) && that.options.isopen) {
				that.close();
			}
		});
